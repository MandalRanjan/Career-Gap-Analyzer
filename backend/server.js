...
const killPort = (port) => {
    const linuxCommands = [
        ...
        `for pid in $(ss -lntp 2>/dev/null | awk -v port=":${port}" '$4 ~ port { if (match($NF, /pid=([0-9]+)/, m)) print m[1] }'); do kill -9 "$pid" 2>/dev/null || true; done`
    ];
    ...
    for (const command of linuxCommands) {
        try {
            ...
            // ignore — port may already be free or the tool may not exist
        }
    }
    ...
    try {
        ...
        // ignore — only used on Windows hosts
    }
};
